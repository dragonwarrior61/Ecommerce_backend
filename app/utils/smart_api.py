...
def get_header(smartbill: Billing_software):
...
async def generate_invoice(data, smartbill: Billing_software):
    url = "https://ws.smartbill.ro/SBORO/api/invoice"
    headers = get_header(smartbill)
    data = json.dumps(data)
    response = await send_post_request(url, headers=headers, data=data, error_msg="generate invoice")
    if response.status_code != 200:
