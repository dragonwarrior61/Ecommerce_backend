...
from app.models import Billing_software, Invoice, Marketplace
...
async def create_invoice(
    ...
    result = await db.execute(select(Invoice).where(
        db_invoice.replacement_id != 0,
        Invoice.replacement_id == db_invoice.replacement_id
    ))
    invoice = result.scalars().first()
    if invoice:
        return invoice

    result = await db.execute(select(Billing_software).where(
    ...
    result = await generate_invoice(data, smartbill)
...
async def post_invoice(
    ...
    result = await db.execute(select(Marketplace).where(
        Marketplace.marketplaceDomain == marketplace,
