...
from app.models import Damaged_good, Internal_Product
...
@router.get('/count')
async def get_damaged_good_count(user_id: int = Depends(get_team_admin_user), db: AsyncSession = Depends(get_db)):
...
async def get_damaged_good(damaged_good_id: int, user_id: int = Depends(get_team_admin_user), db: AsyncSession = Depends(get_db)):
...
async def update_damaged_good(
    damaged_good_id: int,
    damaged_good: Damaged_goodUpdate,
    user_id: int = Depends(get_team_admin_user),
    db: AsyncSession = Depends(get_db)
):
    result = await db.execute(select(Damaged_good).where(Damaged_good.id == damaged_good_id, Damaged_good.user_id == user_id))
...
async def delete_damaged_good(damaged_good_id: int, user_id: int = Depends(get_team_admin_user), db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Damaged_good).where(Damaged_good.id == damaged_good_id, Damaged_good.user_id == user_id))
    damaged_good = result.scalars().first()
