...
async def create_scan_awb(scan_awb: Scan_awbCreate, db: AsyncSession = Depends(get_db)):
    ...
    result = await db.execute(
        select(Scan_awb).where(
            or_(
                Scan_awb.awb_number == awb_number,
                Scan_awb.awb_number == awb_number[:-3]
            )
        )
    )
    ...
    result = await db.execute(select(Returns).where(or_(
        Returns.awb == awb_number,
        Returns.awb == awb_number[:-3]
    )))
...
@router.get('/count')
async def get_scan_awb_count(user_id: int = Depends(get_team_admin_user), db: AsyncSession = Depends(get_db)):
...
async def get_scan_awbs(
    ...
    ).where(Scan_awb.user_id == user_id)
    query = query.outerjoin(
        ReturnsAlias,
        and_(Scan_awb.awb_number == ReturnsAlias.awb)
    )
    query = query.outerjoin(
        AWBAlias,
        AWBAlias.awb_number == Scan_awb.awb_number
    )
    query = query.outerjoin(
        OrderAlias,
        and_(
            OrderAlias.id == AWBAlias.order_id,
            OrderAlias.user_id == AWBAlias.user_id,
            AWBAlias.number > 0
        )
    )
    query = query.outerjoin(
        ReplacementAlias,
        and_(
            ReplacementAlias.order_id == AWBAlias.order_id,
            ReplacementAlias.user_id == AWBAlias.user_id,
            AWBAlias.number < 0
        )
    )
    query = query.outerjoin(
        InvoiceAlias,
        and_(
            InvoiceAlias.order_id == OrderAlias.id,
            InvoiceAlias.user_id == OrderAlias.user_id
        )
    )
    query = query.outerjoin(
        Reverse_InvoiceAlias,
        and_(
            Reverse_InvoiceAlias.order_id == OrderAlias.id,
            Reverse_InvoiceAlias.user_id == OrderAlias.user_id
        )
    )
        ...
        if return_info:
            ...
            for product_id in product_id_list:
                result = await db.execute(select(Product).where(
                    ...
                    Product.product_marketplace == return_info.return_market_place,
...
async def get_scan_awb(scan_awb_id: int, user_id: int = Depends(get_team_admin_user), db: AsyncSession = Depends(get_db)):
...
async def update_scan_awb(
    scan_awb_id: int,
    scan_awb: Scan_awbUpdate,
    user_id: int = Depends(get_team_admin_user),
    db: AsyncSession = Depends(get_db)
):
    result = await db.execute(select(Scan_awb).where(Scan_awb.id == scan_awb_id, Scan_awb.user_id == user_id))
...
async def delete_scan_awb(scan_awb_id: int, user_id: int = Depends(get_team_admin_user), db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Scan_awb).where(Scan_awb.id == scan_awb_id, Scan_awb.user_id == user_id))
    scan_awb = result.scalars().first()
    if scan_awb is None:
        raise HTTPException(status_code=404, detail="scan_awb not found")
